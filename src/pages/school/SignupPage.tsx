    ...
    return (
        <div className="min-h-screen bg-[#FAFBFF] flex flex-col items-center justify-center p-4">
            <div className="mb-8 flex flex-col items-center">
                ...
                <h1 className="text-2xl font-black text-foreground uppercase tracking-tight">Личность ПЛЮС</h1>
            ...
            <Card className="w-full max-w-lg rounded-[40px] border-2 border-border shadow-2xl overflow-hidden bg-background">
                ...
                <CardContent className="p-10 space-y-8">
                    <form onSubmit={handleSignup} className="space-y-6">
                        <div className="space-y-3">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground pl-1">Как вас зовут?</Label>
                            <Input
                                placeholder="ФИО полностью"
                                value={fullName}
                                onChange={(e) => setFullName(e.target.value)}
                                className="h-14 rounded-2xl border-2 font-bold px-6 bg-background"
                                required
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-3">
                                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground pl-1">Пароль</Label>
                                <Input
                                    type="password"
                                    placeholder="••••••••"
                                    value={password}
                        ...
                        <div className="space-y-4">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground pl-1">Ваша роль в школе</Label>
                            <RadioGroup value={role} onValueChange={setRole} className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <div className="relative">
                                    <RadioGroupItem value="student" id="student" className="sr-only" />
                                    <Label
                                        htmlFor="student"
                                        className={`flex flex-col items-center justify-center p-4 rounded-3xl border-2 transition-all cursor-pointer ${role === 'student' ? 'border-primary bg-primary/5 text-primary' : 'border-border bg-background text-muted-foreground hover:border-primary/30'}`}
                                    >
                                ...
                                <div className="relative">
                                    ...
                                    <Label
                                        htmlFor="parent"
                                        className={`flex flex-col items-center justify-center p-4 rounded-3xl border-2 transition-all cursor-pointer ${role === 'parent' ? 'border-primary bg-primary/5 text-primary' : 'border-border bg-background text-muted-foreground hover:border-primary/30'}`}
                                    >
                                ...
                                <div className="relative">
                                    ...
                                    <Label
                                        htmlFor="teacher"
                                        className={`flex flex-col items-center justify-center p-4 rounded-3xl border-2 transition-all cursor-pointer ${role === 'teacher' ? 'border-primary bg-primary/5 text-primary' : 'border-border bg-background text-muted-foreground hover:border-primary/30'}`}
                                    >
                ...
                <CardFooter className="p-10 pt-0 bg-muted/30 flex justify-center">
            ...
            <div className="mt-10 text-muted-foreground font-bold text-xs uppercase tracking-widest">
