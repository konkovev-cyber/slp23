    ...
    return (
        <SchoolLayout title="Журнал преподавателя">
            ...
            <div className="space-y-8 pb-10">
                {/* Header Controls */}
                {/* Header Controls */}
                <div className="flex flex-col xl:flex-row gap-6 justify-between items-start xl:items-center bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm">
                    <div className="flex flex-col md:flex-row gap-4 w-full xl:w-auto">
                        <div className="space-y-1.5">
                            <Label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 pl-1">Выбор дисциплины</Label>
                            <Select value={selectedAssignmentId} onValueChange={setSelectedAssignmentId}>
                                <SelectTrigger className="w-full md:w-80 h-10 rounded-xl border border-slate-200 font-bold text-slate-900 bg-slate-50/50 shadow-sm group text-sm">
                                    <div className="flex items-center gap-2">
                                        <BookOpen className="w-4 h-4 text-primary" />
                                        <SelectValue placeholder="Выберите класс..." />
                                    </div>
                                </SelectTrigger>
                                <SelectContent className="rounded-xl border p-1 bg-white shadow-lg">
                                    {assignments.map((a) => (
                                        <SelectItem key={a.id} value={a.id.toString()} className="h-10 rounded-lg font-medium px-3 hover:bg-slate-50 text-sm">
                        ...
                        <div className="space-y-1.5">
                            <Label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 pl-1">Дата занятия</Label>
                            <div className="relative">
                                ...
                                <Input
                                    ...
                                    className="h-10 rounded-xl border font-bold text-slate-900 bg-slate-50/50 pl-10 w-full md:w-48 shadow-sm text-sm"
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-4 w-full xl:w-auto items-center">
                        ...
                        <Button className="h-10 rounded-xl gap-2 font-bold px-5 bg-slate-900 shadow-md text-sm hover:translate-y-[-1px] transition-all">
                ...
                {/* Students List */}
                <Card className="border border-slate-100 rounded-[24px] overflow-hidden shadow-sm bg-white transition-all">
                    <CardHeader className="p-6 border-b bg-slate-50/30">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-slate-900">Успеваемость</h3>
                                    <CardDescription className="font-medium text-slate-500 text-sm flex items-center gap-2">
                            ...
                            <div className="flex gap-4">
                                <span className="bg-white px-4 py-2 rounded-lg border border-slate-100 font-bold text-slate-400 uppercase tracking-wider text-[9px] flex items-center gap-1.5">
                    ...
                    <CardContent className="p-0">
                        {loading ? (
                            ...
                            <div className="py-20 text-center space-y-4">
                                ...
                                <p className="text-lg font-bold text-slate-300 uppercase tracking-wider">Класс пуст</p>
                            </div>
                        ) : (
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow className="bg-slate-50/50">
                                            <TableHead className="py-4 px-6 font-bold text-[10px] uppercase tracking-wider text-slate-400">Студент</TableHead>
                                            <TableHead className="py-4 px-6 font-bold text-[10px] uppercase tracking-wider text-slate-400">Оценка за день</TableHead>
                                            <TableHead className="py-4 px-6 text-right font-bold text-[10px] uppercase tracking-wider text-slate-400">Управление</TableHead>
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {students.map((student) => (
                                            <TableRow key={student.auth_id} className="group hover:bg-slate-50/30 border-b border-slate-50 last:border-0 transition-all">
                                                <TableCell className="py-4 px-6">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div className="flex flex-col">
                                                            <span className="font-bold text-slate-900 text-sm group-hover:text-primary transition-colors">{student.full_name}</span>
                                                            <span className="text-[9px] uppercase font-bold tracking-wider text-slate-400 flex items-center gap-1.5">
                                                ...
                                                <TableCell className="py-4 px-6">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <Button
                                                            ...
                                                            className="w-9 h-9 rounded-lg border-2 border-dashed border-slate-100 text-slate-300 hover:text-primary hover:border-primary/30 hover:bg-primary/5 transition-all p-0"
                                                        >
                                                ...
                                                <TableCell className="py-4 px-6 text-right">
                                                    <Button
                                                        variant="outline"
                                                        className="h-8 rounded-lg font-bold text-[9px] uppercase tracking-wider border border-slate-200 hover:border-primary/30 hover:bg-primary/5 hover:text-primary transition-all gap-1.5 px-3"
                                                        onClick={() => fetchHistory(student)}
                                                    >
            ...
            <Dialog open={isGradeOpen} onOpenChange={setIsGradeOpen}>
                <DialogContent className="rounded-[24px] border p-6 max-w-sm bg-white shadow-xl">
                    <DialogHeader>
                        <DialogTitle className="text-2xl font-black mb-1">Оценка</DialogTitle>
                        <DialogDescription className="text-base font-medium text-slate-500 flex items-center gap-2">
                            <span className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 text-[10px] font-bold">
                    ...
                    <div className="space-y-6 py-4">
                        <div className="space-y-3 text-center">
                            <Label className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Выберите результат</Label>
                        ...
                        <div className="space-y-2">
                            <Label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 pl-1">Комментарий</Label>
                            <div className="relative">
                                ...
                                <Input
                                    ...
                                    className="h-10 rounded-xl border font-medium px-9 focus:ring-2 focus:ring-primary/5 transition-all text-slate-700 text-sm"
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <DialogFooter>
                        <Button
                            ...
                            disabled={saving}
                            className="w-full h-12 rounded-xl bg-slate-900 text-white font-bold text-lg shadow-lg shadow-slate-300/50 hover:translate-y-[-1px] active:translate-y-[0] transition-all"
                        >
            ...
            <Dialog open={isHistoryOpen} onOpenChange={setIsHistoryOpen}>
                <DialogContent className="rounded-[24px] border p-0 max-w-2xl bg-white overflow-hidden shadow-2xl">
                    <div className="h-24 bg-slate-900 flex items-center px-8 gap-4 relative overflow-hidden">
                        ...
                        <Avatar className="w-12 h-12 border-2 border-white/20 shadow-lg rounded-xl">
                            ...
                            <AvatarFallback className="font-bold bg-white/10 text-white text-sm">
                                {selectedStudent?.full_name?.[0]}
                            </AvatarFallback>
                        </Avatar>
                        <div className="flex flex-col">
                            <h4 className="text-xl font-black text-white">{selectedStudent?.full_name}</h4>
                            <p className="text-slate-400 font-bold uppercase tracking-wider text-[9px] mt-0.5">
                        ...
                        {historyLoading ? (
                            ...
                            <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
                                {studentHistory.map((g) => (
                                    <div key={g.id} className="group relative flex items-center justify-between bg-slate-50 p-4 rounded-2xl border border-slate-100 hover:bg-white hover:border-primary/20 transition-all">
                                        <div className="flex gap-4 items-center">
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <p className="text-[11px] font-medium text-slate-500 max-w-sm">
                    ...
                    <div className="bg-slate-50 p-6 border-t border-slate-100 flex items-center justify-between">
                        <div className="flex gap-4">
                            <div className="flex flex-col">
                                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Всего оценок</span>
                                <span className="font-bold text-slate-900 text-sm">{studentHistory.length}</span>
                            </div>
                        </div>
                        <Button
                            className="h-10 rounded-xl gap-2 font-bold px-6 bg-white border border-slate-200 text-slate-900 hover:bg-slate-50 transition-all shadow-sm text-sm"
                            onClick={() => {
                                if (selectedStudent) {
