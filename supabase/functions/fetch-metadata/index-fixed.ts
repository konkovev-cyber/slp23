...
async function getTextWithEncoding(res: Response): Promise<string> {
  ...
  try {
    ...
    console.log("DEBUG: UTF-8 text length:", text.length);
    console.log("DEBUG: First 50 chars:", text.slice(0, 50));
    return text;
  } catch (e) {
    // Fallback на windows-1251
    console.log("DEBUG: UTF-8 failed, trying Windows-1251");
...
async function fetchVKPost(url: string): Promise<{
  ...
  try {
    ...
    console.log("DEBUG: VK API response status:", data.error ? "ERROR" : "OK");
    ...
    const content = post.text ? decodeHtml(post.text) : "";
