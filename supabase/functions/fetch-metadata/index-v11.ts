...
function decodeWindows1251(buffer: ArrayBuffer): string {
  ...
  for (let i = 0; i < bytes.length; i++) {
    ...
    // ASCII (0-127) оставляем как есть
    if (byte < 128) {
      result.push(byte);
    } 
    // Windows-1251 (128-255) мапим на Unicode
    else {
      // Русские буквы и символы Windows-1251
  ...
  try {
    ...
    if (contentType.includes('windows-1251') || contentType.includes('cp1251')) {
      ...
      try {
        ...
        if (html.includes('\uFFFD')) {
          console.log("DEBUG: UTF-8 has replacement chars, trying Windows-1251");
          html = decodeWindows1251(arrayBuffer);
        }
      } catch (e) {
    ...
    for (const match of imgMatches) {
      ...
      // Заменяем http на https для mixed content
