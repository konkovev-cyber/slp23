 ...
 * Работает для публичных сообществ
 */
async function fetchVKPost(url: string): Promise<{
  ...
  try {
    ...
    // Поддерживаем разные форматы URL VK:
    // 1. vk.com/wall-226860244_207
    // 2. vk.com/club123?w=wall-226860244_207
    // 3. vk.com/id123?w=wall-226860244_207
    // 4. vk.com/wall-226860244_207/z...
    
    let wallMatch = url.match(/wall(-?\d+)_(\d+)/);
    ...
    // wall.get НЕ требует токена для публичных сообществ!
    const apiUrl = `https://api.vk.com/method/wall.get?owner_id=${ownerId}&count=100&v=${VK_VERSION}`;
    console.log("DEBUG: Calling VK API (wall.get, no token):", apiUrl);
    ...
    console.log("DEBUG: Raw response (first 300):", responseText.substring(0, 300));
    ...
    if (!post) {
      console.log("DEBUG: Post not found in last 100 posts");
      // Пробуем найти в copy_history (репосты)
      for (const item of data.response?.items || []) {
        if (item.copy_history?.some((c: any) => c.id === parseInt(postId))) {
          const copiedPost = item.copy_history.find((c: any) => c.id === parseInt(postId));
          if (copiedPost) {
            console.log("DEBUG: Found post in copy_history");
            return parsePost(copiedPost);
          }
        }
      }
      return null;
