...
/**
 * Парсинг VK через HTML (единственный способ без токена)
 */
async function fetchVKPost(url: string): Promise<{
  ...
  try {
    ...
    // Получаем HTML — читаем как UTF-8, потом конвертируем
    const arrayBuffer = await response.arrayBuffer();
    
    // Пробуем определить кодировку из meta тега
    const metaCharset = new TextDecoder('utf-8').decode(arrayBuffer.slice(0, 2000))
      .match(/<meta[^>]+charset=["']?([^"'\s>]+)/i);
    
    let html = "";
    if (metaCharset && metaCharset[1].toLowerCase().includes('windows-1251')) {
      console.log("DEBUG: Detected Windows-1251 from meta charset");
    ...
    // Ищем текст поста в data-attributes
    ...
    for (const selector of textSelectors) {
      ...
      if (match) {
        const raw = stripHtml(decodeHtml(match[1]));
        if (raw.length > 20) {
          content = raw;
          console.log("DEBUG: Found text content, length:", content.length);
          break;
        }
      }
    }
    ...
    for (const match of dataSrcMatches) {
      let imgUrl = match[1];
      if (imgUrl.includes('userapi.com') || imgUrl.includes('vk.com')) {
        imgUrl = imgUrl.replace('http://', 'https://');
...
function isGarbageImage(url: string): boolean {
  const patterns = [/emoji/i, /icon/i, /favicon/i, /avatar/i, /logo/i, /pixel/i, /ads/i];
