...
async function fetchVKPost(url: string): Promise<{
  ...
  try {
    ...
    // Получаем ArrayBuffer и декодируем явно в UTF-8
    const arrayBuffer = await response.arrayBuffer();
    const textDecoder = new TextDecoder('utf-8');
    const responseText = textDecoder.decode(arrayBuffer);
    
    console.log("DEBUG: Response text (first 200):", responseText.slice(0, 200));
    ...
    console.log("DEBUG: Content (first 100):", content.slice(0, 100));
